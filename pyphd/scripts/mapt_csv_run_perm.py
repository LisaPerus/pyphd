...
from pyphd.constants import (ANALYSES_DETAILS_JSON, MODELS_COVARIATES,
                             RSFMRI_TEMPORAL_COVARIATES, MAPT_RSFMRI_COV_TYPES)
...
def get_cmd_line_args():
    ...
    parser.add_argument(
        "-R", "--report-specific-contrast-pval", type=str,
        help="Report pvalue for a specific contrast.")
    parser.add_argument(
    ...
    parser.add_argument(
        "-U", "--nb-cpu",  type=int, default=1,
        help="Nb of CPU. If > 1, use multiprocessing.")
    parser.add_argument(
        "-V", "--verbose",
...
for idx_tp, tp in enumerate(inputs["timepoints"]):
    ...
    for model in inputs["models"]:
        ...
        for model_spe, model_spe_data in all_model_information.items():
            ...
            # (see http://mumford.fmripower.org/mean_centering/ and
            # https://www.jiscmail.ac.uk/cgi-bin/webadmin?A2=fsl;b0aa0023.1412)
            ...
            for var in var_to_test:
                ...
                palm_outputs[var] = palm_output_basename

            results = {}
            if inputs["nb_cpu"] > 1:
                pool = mp.Pool(inputs["nb_cpu"])

                # > Note : results are not ordered
                results_apply_sinc = [pool.apply_async(palm, args=(
                           palm_inputs[var],
                           design_mat_file,
                           constrat_con_file,
                           f_contrast_file,
                           ...
                           inputs["nb_perms"],
                           inputs["two_tail"],
                           inputs["save_parametric"],
                           inputs["palm_path"])) for var in var_to_test]
                pool.close()
                ...
                # > Store results
                for res in results_apply_sinc:
                    all_elts = res.get()
                    var = os.path.basename(all_elts[0][0]).split("_dat")[0]
                    var = var[len("palm_"):]
                    results[var] = list(all_elts)
            else:
                for var in var_to_test:
                    ...
                    (palm_tstat_val_files, palm_fstat_val_files,
                     ...
                     palm_time_elasped_file, palm_config_file, cmd) = palm(
                        indata=palm_inputs[var],
                        design_file=design_mat_file,
                        contrast_file=constrat_con_file,
                        f_contrast=f_contrast_file,
                        output_basename=palm_outputs[var],
                    ...
                    results[var] = [palm_tstat_val_files, palm_fstat_val_files,
            ...
            for var, res in results.items():
                palm_tstat_val_files = res[0]
                palm_fstat_val_files = res[1]
                palm_tstat_pval_unc_files = res[2]
                palm_fstat_pval_unc_files = res[3]
                palm_p_tstat_fwe_files = res[4]
                palm_p_fstat_fwe_files = res[5]
                palm_p_tstat_uncparap_files = res[6]
                palm_p_fstat_uncparap_files = res[7]
                palm_time_elasped_file = res[8]
                palm_config_file = res[9]
                cmd = res[10]
                ...
                for output_var in [palm_tstat_val_files, palm_fstat_val_files,
                    ...
                    for varname_data in local_variables.items():
                        if type(varname_data[1]) == type(output_var):
                            if varname_data[1] is output_var:
                                output_varname = varname_data[0]

                    # Find the problem : not returning fstat file?
                    output_var_list = []
            ...
            pvalues = []
            pvalues_fdr_corr = []
            stat_chosen_for_pvalues = []
            stat_value_for_stat_chosen_for_pvalues = []
            stat_chosen_for_pvalues_uncparap = []
            for var in var_to_test:

                # >> Check if a specific contrast was specificied to report
                # pvalue.
                # Multiple contrasts can be provided by default
                # (e.g 1 -1 and -1 1 for ttest, which is the same if we put
                # twotail option) and we can choose to report only one
                if inputs["report_specific_contrast_pval"] is not None:
                    all_pval_files = outputs[
                        "palm_fstat_pval_unc_files"][tp][var] + outputs[
                            "palm_tstat_pval_unc_files"][tp][var]
                    for fid in all_pval_files:
                        if "_uncp_" + inputs[
                                "report_specific_contrast_pval"] in fid:
                            pval_final_file = fid
                    with open(pval_final_file, "rt") as open_file:
                        pval = open_file.readlines()[0].strip("\n")
                else:
                    ...
                    # > Check that there is no confusion between multiple
                    # contrasts to choose results from
                    if len(pval) > 1:
                            raise ValueError(
                                "Multiple contrast to choose pval from")
                    else:
                        pval = pval[0]

                    # > Find other details
                    if is_fstat:
                        pval_final_file = outputs[
                            "palm_fstat_pval_unc_files"][tp][var][0]
                    else:
                        pval_final_file = outputs[
                            "palm_tstat_pval_unc_files"][tp][var][0]

                # Append pval
                pvalues.append(pval)

                # > Find other details
                stat_chosen_for_pvalues.append(
                    os.path.basename(pval_final_file.replace("uncp_", "")))
                with open(pval_final_file.replace(
                          "uncp_", ""), "rt") as open_file:
                    stat_value_for_stat_chosen_for_pvalues.append(
                        open_file.readlines()[0].strip("\n"))
                uncparap_file = pval_final_file.replace(
                    "_uncp_", "_uncparap_")
                all_uncparap_files = outputs[
                    "palm_p_tstat_uncparap_files"][tp][var]
                all_uncparap_files += outputs[
                    "palm_p_fstat_uncparap_files"][tp][var]
                if uncparap_file in all_uncparap_files:
                    with open(uncparap_file, "rt") as open_file:
                        stat_chosen_for_pvalues_uncparap.append(
                            open_file.readlines()[0].strip("\n"))
                else:
                    stat_chosen_for_pvalues_uncparap.append("NA")

            # Assemble all pvalues and correct with FDR
            ...
            with open(stat_outfile, "wt") as open_file:
                header = ["Variables", "Test", "T-value", "F-value",
                          "Statistic for which pval is reported",
                          "Statistic for which pval is reported (value)",
                          "Statistic parametric P-value", "P-value",
                ...
                for idx_var, var in enumerate(var_to_test):
                    ...
                    line += [stat_chosen_for_pvalues[idx_var]]
                    line += [str(
                        stat_value_for_stat_chosen_for_pvalues[idx_var])]
                    line += [str(stat_chosen_for_pvalues_uncparap[idx_var])]
