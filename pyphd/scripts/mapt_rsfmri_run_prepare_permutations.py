...
prepare_permutation_script = os.path.join(
    "/home/lp259104/PHD/SCRIPTS/GIT_REPOS/pyphd/pyphd/scripts", "pyphd_prepare_permutation_connectivities.py")
...
for tp in inputs["timepoints"]:
    ...
    if not os.path.isdir(tp_dir):
        os.mkdir(tp_dir)
    model_dir = os.path.join(tp_dir, inputs["models"][0])
...
for tp in inputs["timepoints"]:
    ...
    if not os.path.isdir(tp_dir):
        os.mkdir(tp_dir)
    model_dir = os.path.join(tp_dir, inputs["models"][1])
    if not os.path.isdir(model_dir):
        os.mkdir(model_dir)
    ...
    conn_file = extract_connectivities(
        inputs["group_name"], tp=tp, center_name=inputs["center"],
        covariates=covariates_extract_conn, outdir=model_dir)
